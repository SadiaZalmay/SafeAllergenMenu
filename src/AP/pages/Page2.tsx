...
const Page2: React.FC = () => {
  ...
  const [page2, setPage2] = useState<Page2Item[]>([]);

  useEffect(() => {
    const fetchAllPage2 = async () => {
      try {
        const res = await axios.get("http://localhost:5000/api/page2/");
        setPage2(res.data);
      } catch (err) {
  ...
  const handleDelete = async (id: number) => {
    try {
      await axios.delete(`http://localhost:5000/api/page2/${id}`);
      // Fetch the updated page2 list after deletion
      const res = await axios.get("http://localhost:5000/api/page2/");
      setPage2(res.data); // Update the page2 state with the new data
    } catch (err) {
